...
SRRcwt<-read.csv("SRRy CWT.csv")
SRRcwt<-SRRcwt %>%
  filter(run_year %in% 2000:2009)
SRR<-rbind(SRR, SRRcwt)
SRR$Rate<-"Spawner Reduction Rate"
colnames(SRR)[1]<-"Year"
ExploitR<-SIdata %>%
  mutate(SRRyMean = ExploitR/100) %>%
  select(Year, SRRyMean)
ExploitR$SRRyLower<-NA
ExploitR$SRRyUpper<-NA
ExploitR$Rate<-"SI Exploitation Rate"
SRR<-rbind(SRR, ExploitR)
ggplot(SRR, aes(x=Year, y = SRRyMean))+
  geom_errorbar(aes(ymin=SRRyLower, ymax=SRRyUpper ))+
  geom_point(size = 3, aes(shape = Rate))+
  ylab("rate")+ 
  xlab("Year")+
  ylim(0,1)+
  scale_shape_manual(values=c(16, 1))+
  scale_x_continuous("Run Year", labels = as.character(2000:2019), breaks = 2000:2019, limits = c(1999.5, 2019.5)) +
  blank_bg
ggsave('Fig4.png', plot=last_plot(),  width=10, height = 4)

###################################################
####### Compare ocean harvest rate to age impacts #
###################################################
Impact<-read.csv("Impact Uncertainty CWT.csv")

ageimpact<-Impact %>% #for reporting age-specific impact results
  filter(brood_year %in% 1998:2016)  %>%
  group_by(Age) %>%
  summarise(mean = mean(ImpMean), sd= sd(ImpMean))

Impact<-Impact %>%
  filter(brood_year %in% 1998:2016)  %>%
  mutate(Year = brood_year + Age) %>%
  select(Year,ImpLower, ImpMean, ImpUpper, Age)
Impact$Age<-as.factor(Impact$Age)
ggplot(Impact, aes(x=Year, y = ImpMean, color = Age))+
  geom_errorbar(aes(ymin=ImpLower, ymax=ImpUpper))+
  geom_point(size = 3, aes(shape = Age))+
  ylab("ocean impact rate")+ 
  xlab("Management Year")+
  ylim(0,1)+
  scale_shape_manual(values=c(0, 15, 17, 2))+
  scale_x_continuous("Management Year", labels = as.character(2000:2020), breaks = 2000:2020, limits = c(1999.5, 2020.5)) +
  scale_color_manual(values=c("#DF7861", "#94B49F","#cfe5ea","#a98f7f"))+
  blank_bg
ggsave('Fig2.png', plot=last_plot(),  width=10, height = 4)
###################################################
